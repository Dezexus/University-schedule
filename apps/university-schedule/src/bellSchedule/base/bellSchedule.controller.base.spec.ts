...
const service = {
  ...
  bellSchedules: () => FIND_MANY_RESULT,
  bellSchedule: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("BellSchedule", () => {
  ...
  test("GET /bellSchedules/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/bellSchedules"}/${nonExistingId}`)
  ...
  test("GET /bellSchedules/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/bellSchedules"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /bellSchedules existing resource", async () => {
